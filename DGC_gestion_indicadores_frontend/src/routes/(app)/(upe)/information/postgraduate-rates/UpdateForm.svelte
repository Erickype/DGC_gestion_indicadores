<script lang="ts">
	...
	import {
		updatePostgraduateCohortListSchema,
		type UpdatePostgraduateCohortListSchema
	} from './schema';
	...
	export let data: SuperValidated<
		Infer<UpdatePostgraduateCohortListSchema>,
		App.Superforms.Message
	>;
	export let comboMessages: Message[][];
	export let updateEntity: PostgraduateCohortList;
	comboMessages = [];
	...
	const form = superForm(data, {
		validators: zodClient(updatePostgraduateCohortListSchema),
		taintedMessage: null,
		onUpdated: ({ form: f }) => {
			...
			if (message.success) {
				...
				return toast.success(`Valores de tasas de cohorte actualizados: ${response.name}`);
			}
			fillForm();
	...
	function fillForm() {
		...
		$formData.year = updateEntity.year;
		$formData.graduated_students = updateEntity.graduated_students;
		$formData.total_students = updateEntity.total_students;
	}
	...
	function addCurrentYear(years: number[]) {
		years.push(updateEntity.year);
		return years;
	}
</script>

<form action="?/updatePostgraduateCohortList" use:enhance>
			...
			<Form.Field {form} name="year" class="flex flex-col">
				...
				{:then missingYears}
					<FormSelect
						...
						comboData={GenerateComboMessagesFromMissingYears(addCurrentYear(missingYears.years))}
